...
class Block:
    def __init__(self, index: int, x_min: float, x_max: float, y_min: float, y_max: float, z_min: float, z_max: float, type: int, material: int, look: int, up: int, color: str):
...
class AvToObjConverter:
    ...
    def generateObjectString(self, vertices: List[Vertex], faces: List[Face],
        ...
        out_string = "o {}\n".format(name)
        smoothing = 1 if smoothing else 0
...
def rotateVectors(vectors: List[Vertex], look: int, up: int):
    """
    Rotates the given vectors to correspond with the given values for look and up.

    :param vectors: List of vectors to rotate
...
def transformBlock(verts: List[List[float]], vert_normals: List[List[float]], look: int, up: int, block=None):
    vertices = verts.copy()
    ...
    for vertex in vertices:
        ...
        temp_result = translate(temp_result, translation_vector[0], translation_vector[1], translation_vector[2])

        temp_result = temp_result.tolist()
        result = [np.round(temp_result[0][0], 15), np.round(temp_result[1][0], 15), np.round(temp_result[2][0], 15)]
        output_vertices.append(result)

    for normal in normals:  # Update the normals to work with the scaling  -- Broken
        normal.append(0)
...
def _getTranslationMatrix(x, y, z):
